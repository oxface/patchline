...
await builder.addNextJsApp("web", "../web").withReference(api).waitFor(api).withBrowserLogs();
...
```ts
await builder.addProject("api", "../Api/Api.csproj").withEndpoint("admin", (e) => {
...
await builder.addViteApp("web", "../web").publishAsPackageScript({ scriptName: "start" });
```
...
await builder
  .addDockerfileBuilder("myimage")
  .withDockerfileBuilder((b) =>
    b
      .from("node:20-alpine")
      .workdir("/app")
      .copy("package*.json", "./")
...
await builder.addProject("api", "../Api/Api.csproj").withComputeEnvironment(aca);
```
...
await builder.addAzureFoundry("foundry").addModel("chat", FoundryModels.OpenAI.Gpt41Mini);
```
...
```ts
await builder.addViteApp("frontend", "../frontend").withBrowserLogs();
```
